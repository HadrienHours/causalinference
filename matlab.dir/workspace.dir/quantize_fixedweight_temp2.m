...
%This fucntion discretizes the input by approximating it with interval functions such that the number of samples falling in each interval is the same
%Note that this is not always possible as (i) if not enough unique values then the algorithm takes each unique value as zero length interval (discrete function) (ii) if continuous values we pick each "weight"-th value as bin edge if there are less than "weight" sample for which the value in one dimension is the same the upper bound of the bin can be defined as this value and more than one sample will be excluded by the strict < from the upper bound of the bin
...
%		weitgh: scalar representing the number of samples by interval, this value is a target value
%output
%		edges: cell containing the sequence of edges for each bin for each dimension and number of samples for each bins
