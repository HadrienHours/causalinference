...
function g = kpc_indtest_clust_ward(X,alpha,alpha_m,alpha_conv,shuff,outputfile)


warning('off');
ms = floor(size(X,1)/200);
T = clusterdata(X,'linkage','ward','maxclust',ms);
clusters = cell(1,ms);
for i=1:ms
   clusters{i} = X(find(T==i),:); 
end
...
for z=1:ms
    ...
    %                 [sig, p] = hsicTestBootIC(X(:,i),X(:,j),alpha,shuffles);
                    sig = (alpha >= (pglobs(i,j)+pglobs(j,i))/2);
                    % if independent
                    if (~sig) 
                        fprintf('%d ind %d | \n', i,j);
                        g(i,j)=0;
                        ...
                        fprintf('Independence test failed between %d and %d with p-value %d \n',i,j,pglobs(j,i));
                    end
                    continue;
                end
    ...
    %                 [sig, p] = hsiccondTestIC(X(:,i),X(:,j),X(:,condSet),alpha,shuffles);
                    try
                        [pval,stat] = indtest_new(X(:,i),X(:,j),X(:,condSet),[]);
                        sig = pval <= alpha;
                    catch ME
                        ME
                        fprintf('Conditional independence test failed with 1st method\n');
                        [sig, p] = hsiccondTestIC(X(:,i),X(:,j),X(:,condSet),alpha,shuffles);
                    end
                    % if independent
                    if (~sig)
               ...
               while (sp>0)
                   ...
                   for j=1:size(combs,1)
                       ...
                       try
                           ...
                           [tr,b] = train_par(a,data(X(:,union(st,i)),X(:,pp(k))),alpha_m,alpha_conv);
                       catch ME
                           ME
                           fprintf('Relevant Vector Machine Regression failed, use of Multi Ridge Regression\n')
    %                     a = svr();
                           a = multi_rr();
                       ...
                       okay = 1;
                       for k=1:length(ppa)
                          [pval, stat] = indtest_new(X(:,i)-tr.X,X(:,ppa(k)),[],[]);
                           ...
                           for l=1:length(undk)
                              ...
                              for m=1:size(kcombs,1)
                                 ...
                                 try
                                     ...
                                     ME
                                     fprintf('The Relevant Vector Machine regression failed, use of Multi Ridge Regression\n');
                                     a = multi_rr();
                                    [tr,b] = train(a,data(X(:,union(st,i)),X(:,pp(k))));
                                 end
    %                              a = svr();
                                 [pval,stat] = indtest_new(X(:,pp(k))-tr.X,X(:,i),[],[]);
