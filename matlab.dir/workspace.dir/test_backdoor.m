function [params,yval,ymean,err_l,size_l] = test_backdoor(ds,deltas,k,b,x,ydim,zdim,yobj,yrange)
...
for error_d = deltas
   for cluster_s = k
      for bin_sp = b
          ...
          params(idx,:)=[error_d,cluster_s,bin_sp];
          [dy_q,prob,dsq_q,dsp_q,edges_q,centroids_q] = backdooreffect_q(ds,x,ydim,zdim,cluster_s,bin_sp,error_d);
          yval{idx} = dy_q;
          proby(idx) = prob;
          I = find(dy_q(:,end) ~= 0);
