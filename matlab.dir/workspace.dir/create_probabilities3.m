function [prob] = create_probabilities3(M,flag)
...
if (flag == 0)
    ...
    for i=1:p
        histos{i} = hist(M(:,i),h(i));
    end
else
    for i=1:p
        edges{i};
        histos{i} = histc(N{i},h(i));
    end 
end
...
% %        fprintf('--------------------------\n\n\n');
%    for i=1:n
%        p = probhistbin(M(i,j),edges{j},histos{j},n);
%        prob{j}(1,i) = M(i,j);
%        prob{j}(2,i) = p;
%    end
% end
...
for i=1:p
   subplot(linef,3,i);
   bar(xis{i},histos{i}'/(sum(histos{i})*binw(i)),'Barwidth',binw(i)); 
   xlabel('values');
   ylabel('frequency');
end
