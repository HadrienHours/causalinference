function [pdag, G] = learn_struct_pdag_pc_log(pathlog,cond_indep, n, k, varargin)
...
if fid == -1
    ...
    fprintf(fid,'Sarting the pc algorithm at %s\n',datestr(clock));
end
...
while ~done
  ...
  [X,Y] = find(G);
  fprintf(fid,'Starting with order %d\n',ord);
  for i=1:length(X)
    x = X(i); y = Y(i);
    fprintf(fid,'Studying the dependence between X%d and X%d\n',x,y);
    %     nbrs2 = mysetdiff(myunion(neighbors(G, x), neighbors(G,y)), [x y])
    ...
    if length(nbrs) >= ord & G(x,y) ~= 0
      ...
      for si=1:length(SS)
        S = SS{si};
%         fprintf('x value: %d, y value: %d, S value: %d\n',x,y,S);
        if ~isempty(S)
            fprintf(fid,'Testing independence between X%d and X%d conditionally on {X%d',x,y,S(1));
            idx = 2;
            while idx <= size(S,2)
                fprintf(fid,', X%d',S(idx));
            end
            fprintf(fid,'}\n');
        else
            fprintf(fid,'Testing independence between X%d and X%d\n',x,y)
        end
        
        if feval(cond_indep, x, y, S, varargin{:})
            fprintf(fid,'Independence found\n');
%           if isempty(S)
