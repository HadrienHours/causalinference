...
%dimension,the corresponding value,prob, cdf and number of samples
...
% 		fprintf('For dimension %d the size of the number of bins is %d\n',i,size(bins,1))
		...
		for j = 1:s
            ...
            %The probability of a value is proportional to the width of the
            %bin that it represents, the wider the bin the least the
            %probability
            lj = bins(j+1) - bins(j);
			edges{i}(j,[1:4,6]) = [bins(j),bins(j+1),binv,ns/(n*lj),ns];
		end
	else
% 		fprintf('Dimension %d is already discrete with size %d\n',i,su)
		...
		for j = 1:su
			...
			edges{i}(j,[1:4,6])=[u(j),u(j),u(j),ns/n,ns];
		end
    end
    %Divide the 4th column so that it sums to 1, in case of the discrete
    %dim rho = 1
    rho = sum(edges{i}(:,4));
    edges{i}(:,4) = edges{i}(:,4)/rho;
    edges{i}(1,5) = edges{i}(1,4);
    for j = 2:size(edges{i},1)
       % Pr(x <= Xi) = Pr(x <= X[i-1]) + Pr(X = Xi)
       edges{i}(j,5) =edges{i}(j-1,5)+edges{i}(j,4);
    end
end 
