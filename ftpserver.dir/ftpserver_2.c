...
/*
Adding socket tracking
Create a file in /tmp/lockfiles
$STATE_socket_$METHOD_$PORT
$STATE one letter 
			C: Created: First file created, the socket created, connection not open yet with Client
			O: Open: Tcpdump is recording
			F: Finish: The transfer is finished
			W: Written, Tcpdump finished the trace
$METHOD two letters
			PO: doport method used
			PA: Passive MEthod used
Hadrien Hours 12/9/2012
*/
...
{
	printf("Entering the socket closing method of the server\n");
	struct sockaddr_in pasvaddr;
        int len;
        unsigned int ip;
        unsigned short port;
        if (cuftpd_pasv_fd >= 0) {
		printf("The pasv1 socket %d is about to be closed\n",cuftpd_pasv_fd);

		len = sizeof(pasvaddr);
	        getsockname(cuftpd_pasv_fd, (struct sockaddr*)&pasvaddr, &len);
        	ip = ntohl(pasvaddr.sin_addr.s_addr);
	        port = ntohs(pasvaddr.sin_port);
		printf("The port found for socket %i is %i\n",cuftpd_pasv_fd,port);
		char *pathname=malloc(snprintf(NULL,0,"/tmp/lockfiles/")+1);
		sprintf(pathname,"%s","/tmp/lockfiles/");
		char *s = NULL;
        	asprintf(&s, "%s%i","F_lock_PO_",port);
		char *filename = malloc(snprintf(NULL,0,"%s%s",pathname,s)+1);
		sprintf(filename,"%s%s",pathname,s);
		//Create the lock signaling port opening
		FILE *fs = fopen(filename,"w");
		fclose(fs);

		if (verbose==1)
		{
			printf("The lock file %s was created\n",filename);
		}
		//Wait for the bash program to ack tcpdump is finished
		int index = strlen("/tmp/lockfiles/");
		filename[index]='W';
		do{
			fs = fopen(filename,"r");
			if(verbose==1){
				printf("Wait 5 seconds for tcpdump to signal it is finished (%s)\n",filename);
			}
			sleep(5);
		}while(fs==0);
		...
		printf("The pasv socket %d is about to be closed\n",cuftpd_pasv_connfd);

		len = sizeof(pasvaddr);
	        getsockname(cuftpd_pasv_connfd, (struct sockaddr*)&pasvaddr, &len);
      		ip = ntohl(pasvaddr.sin_addr.s_addr);
        	port = ntohs(pasvaddr.sin_port);
		printf("The port found for socket %i is %i\n",cuftpd_pasv_connfd,port);
		...
		char *s = NULL;
		 ...
		 if (verbose==1)
                ...
                //Wait for the bash program to ack tcpdump is open and running 
		int index = strlen("/tmp/lockfiles/");
                filename[index]='W'; 
                do{
                        fs = fopen(filename,"r");
		...
		printf("The port socket %d is about to be closed\n",cuftpd_port_connfd);

		len = sizeof(pasvaddr);
	        getsockname(cuftpd_port_connfd, (struct sockaddr*)&pasvaddr, &len);
        	ip = ntohl(pasvaddr.sin_addr.s_addr);
	        port = ntohs(pasvaddr.sin_port);
		printf("The port found for socket %i is %i\n",cuftpd_port_connfd,port);
		...
		char *s = NULL;
        	asprintf(&s, "%s%i","F_lock_PO_",port);
                char *filename = malloc(snprintf(NULL,0,"%s%s",pathname,s)+1);
                sprintf(filename,"%s%s",pathname,s);
                //Create the lock signaling port closing
                FILE *fs = fopen(filename,"w");
                fclose(fs);
		 ...
		 if (verbose==1)
                ...
                //Wait for the bash program to ack tcpdump has finished recording  
		int index = strlen("/tmp/lockfiles/");
                filename[index]='W';
                do{
                        fs = fopen(filename,"r");
...
cuftpd_do_port(int ctrlfd, char *cmdline)
{
	...
	//Test variables to test getsockname method
	struct sockaddr_in pasvaddr_tmp;
        int len_tmp;
        unsigned int ip_tmp;
        unsigned short port_tmp;
	//end

	printf("Entering do_port method\n");
        CUFTPD_CHECK_LOGIN();
	...
	printf("The port socket number is %d and Client port %d\n",cuftpd_port_connfd,ntohs(port));
	...
	//Get local socket infos
	printf("Entering getsockame method\n");
	len_tmp = sizeof(pasvaddr_tmp);
	getsockname(cuftpd_port_connfd, (struct sockaddr*)&pasvaddr_tmp, &len_tmp);
        ip_tmp = ntohl(pasvaddr_tmp.sin_addr.s_addr);
        port_tmp = ntohs(pasvaddr_tmp.sin_port);
        printf("The port found by getsockname for socket %i is %i\n",cuftpd_port_connfd,port_tmp);

	//We add +1 for the trailing end of character
	char *pathname=malloc(snprintf(NULL,0,"/tmp/lockfiles/")+1);
	sprintf(pathname,"%s","/tmp/lockfiles/");
	printf("Path name string created %s\n",pathname);
	char *s = NULL;
	asprintf(&s, "%s%i","C_lock_PO_",port_tmp);
	printf("file name created %s\n",s);
	char *filename = malloc(snprintf(NULL,0,"%s%s",pathname,s)+1);
	sprintf(filename,"%s%s",pathname,s);
	//Create the lock signaling port opening
	...
	CUFTPD_DEBUG("Lock file created %s\n",filename);
...
err_label:
        if (cuftpd_port_connfd >= 0) {
                close(cuftpd_port_connfd);
		printf("The port socket %d had been closed\n",cuftpd_port_connfd);
		char *pathname=malloc(snprintf(NULL,0,"/tmp/lockfiles/")+1);
		sprintf(pathname,"%s","/tmp/lockfiles/");
		char *s = NULL;
        	asprintf(&s, "%s%i","F_lock_PO_",port);
...
cuftpd_do_pasv(int ctrlfd, char *cmdline)
{
	...
	printf("PASV Socket created with id %d\n",cuftpd_pasv_fd);
        if (cuftpd_pasv_fd < 0) {
                CUFTPD_DEBUG("socket() failed: %s\n", strerror(errno));
                return cuftpd_send_resp(ctrlfd, 550);
        }
	...
	char *pathname=malloc(snprintf(NULL,0,"/tmp/lockfiles/")+1);
        sprintf(pathname,"%s","/tmp/lockfiles/");
	char *s = NULL;
        asprintf(&s, "%s%i","C_lock_PO_",port);
        char *filename = malloc(snprintf(NULL,0,"%s%s",pathname,s)+1);
        sprintf(filename,"%s%s",pathname,s);
        //Create the lock signaling port opening
        FILE *fs = fopen(filename,"w");
...
cuftpd_do_retr(int ctrlfd, char *cmdline)
{
	...
	printf("Entering do_retr method\n");
...
main(int argc, char *argv[])
{
	...
	char *pathname=malloc(snprintf(NULL,0,"/tmp/lockfiles/")+1);
        sprintf(pathname,"%s","/tmp/lockfiles/");
	int code_err = mkdir(pathname,00777);
