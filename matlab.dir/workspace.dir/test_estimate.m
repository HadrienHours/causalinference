function [zmin,zmax,mlist_clean,mlist,zerrmin,zerrmax,zerravg] = test_estimate(Xe,Ye,Ze,Xr,Yr,Zr,dx,dy)
...
err = abs(zclean - Zr);
%for i = 1:size(zflat,2)
%	err(i) = abs(zflat(i) - Zr);
%end
fprintf('Error computed with size %d,%d\n',size(err))
%pause
