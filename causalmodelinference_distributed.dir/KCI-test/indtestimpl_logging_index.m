function [res,p] = indtestimpl_logging_index(i,j,k,ds,alpha,pathdir,indextest)
%This function is a wrapper of indtest_new to return 1 in independence case
% 0 otherwise. This function uses the median by default but store all the
% intermediary results in csvfiles
%usage
...
if isempty(k)
    fileout=strcat(pathdir,'/independence_',num2str(i),'_',num2str(j),'_cond_0_',num2str(indextest),'.csv');
else
    fileout=strcat(pathdir,'/independence_',num2str(i),'_',num2str(j),'_cond');
    for d = 1:size(k,2)
        fileout = strcat(fileout,'_',num2str(k(d)));
    end
    fileout = strcat(fileout,'_',num2str(indextest),'.csv');
end
