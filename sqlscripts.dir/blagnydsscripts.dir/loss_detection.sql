...
BEGIN
	...
	biggest := 0;
	max_ipid := -1;
	execute 'select '''||tbl_t||'''||''_rtts'' ' into viewname;
	execute 'CREATE TEMPORARY VIEW '||viewname||' as (select * from rtts('||cid_t||','||dir_t||','''||tbl_t||''') as t(ts timestamp, rtt interval))';
	execute 'select avg(rtt) from '||viewname||' ' into rtt_t;
...
--	execute 'select CAST( CAST(stddev(extract(SECONDS from rtt)) as VARCHAR) as interval) from rtts('||cid_t||','||dir_t||','''||tbl_t||''') as t(ts timestamp, rtt interval)' into std_t;
	execute 'select CAST( CAST(stddev(extract(SECONDS from rtt)) as VARCHAR) as interval) from '||viewname||' ' into std_t;
	rto_t := rtt_t+4*std_t;
...
--			execute 'select rtt from rtts('||cid_t||','||dir_t||','''||tbl_t||''') as t(ts timestamp, rtt interval) where ts <= '''||row.ts||''' order by ts DESC limit 1' into rtt_m;
			execute 'select rtt from '||viewname||' where ts <= '''||row.ts||'''order by ts DESC limit 1' into rtt_m;
...
--				execute 'select rtt from rtts('||cid_t||','||dir_t||','''||tbl_t||''') as t(ts timestamp, rtt interval) where ts >= '''||row.ts||''' limit 1' into rtt_m;
				execute 'select rtt from '||viewname||' where ts >= '''||row.ts||''' limit 1' into rtt_m;
