function [bd_dsy,prob,dsq,dscen,edges] = general_backdooreffect_q_fixed_centroid(ds,dscen,x,y,z,w,b,delta)
%This function return the Probability of Pr(Y / do(X=x),W=w) using the backdoor
%adjustement with the set z Pr(Y / do(X=x),W=w) = sum_{z}Pr(Y/X=x,W=w,Z=z)Pr(Z=z)
...
%       probability
%       x = matrix of sx rows, 2 column. Each row contains the dimension 
%       and the value to set on this dimension
%       y = scalar representing the dimension of Y
%       z = vector representing the dimension(s) corresponding to the
%       blocking set Z
%       w = sw*2 matrix representing the conditioning variables
